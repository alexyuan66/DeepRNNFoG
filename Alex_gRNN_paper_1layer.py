...
RNN_cell1 = SimpleRNN(n_a, activation=af, input_shape=(Tx, n_values))

def parse_commandline():
    global RNN_cell1, af, Tx, predict_d, n_a, Tstep, Tcutoff, Sstep
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == 'SimpleRNN' :
            ...
            RNN_cell1 = LSTM(n_a, activation=af, input_shape=(Tx, n_values))
            #RNN_cell2 = LSTM(n_a, activation=af)
        elif sys.argv[1] == 'GRU' :
            RNN_cell1 = GRU(n_a, activation=af, input_shape=(Tx, n_values))
...
print(YYY[0][0], ' ', YYY[0][1])
print(YYY[1][0], ' ', YYY[1][1])

#(ri, rj, rk) = XXX.shape

#for i in range(ri):
#    print(i, ' ', j, ' : ', XXX[i][0][0], ', ', XXX[i][0][8],  XXX[i][50][0], ', ', XXX[i][50][8], XXX[i][99][0], ', ', XXX[i][99][8])
...
print(YYY[0][0], ' xx ', YYY[0][1])
print(YYY[1][0], ' xx ', YYY[1][1])

#for i in range(Out1.shape[0]):
#    print(i, ': [', YYY[i][0], ', ', YYY[i][1], '] != [', Out1[i][0], ', ', Out1[i][1], ']')
