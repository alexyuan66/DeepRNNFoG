...
# May 15, 2020
#
# June 8, 2020: fixed the data partition to not be in the middle of an episode
...
def gen_batch_one_episode(data, beg, end, shuffle = 1):
    ...
    for i in [n for n in range(rbeg, end-Tx*Tstep, Sstep)]:
        ...
        if i == rbeg :
            Y += [[1, 0]] # marker
        elif i+(Tx-1)*Tstep+predict_d > end-1:
...
#                print(row.strip().split(' ')[1:])
            ...
            par = XXX*int(len(X_signals)/YYY)
            # print('par = ', par)

            if X_signals[par][9] == '2' :
                while par < len(X_signals) and X_signals[par][9] == '2' :
                    par += 1
            elif X_signals[par][9] == '1' :
                while par >= 0 and X_signals[par][9] == '1' :
                    par -= 1
                par += 1
            else :
...
raw_train_X, raw_train_Y, raw_test_X, raw_test_Y = load_data(TRAIN+TFILE, shuffle = 0)
if TFILE2 != '' and TFILE2 != TFILE :
    raw_train_X += raw_test_X
    raw_train_Y += raw_test_Y
    raw_train_X1, raw_train_Y1, raw_test_X, raw_test_Y = load_data(TRAIN+TFILE2, shuffle = 0)
    if (XXXYYY2 == 0) :
        if (RNNfunc != 'transfer1' and RNNfunc != 'transfer2' and RNNfunc != 'transfer3' and RNNfunc != 'load') : 
            raw_test_X = raw_train_X1 + raw_test_X
            raw_test_Y = raw_train_Y1 + raw_test_Y
    else :
        raw_train_X = raw_train_X + raw_train_X1
        raw_train_Y = raw_train_Y + raw_train_Y

    if RNNfunc == 'transfer1' or RNNfunc == 'transfer2' or RNNfunc == 'transfer3':
        raw_train_X = raw_train_X1
        raw_train_Y = raw_train_Y1
...
train_X, train_Y = remake_training_set(raw_train_X, raw_train_Y, datasettype, classratio)
test_X, test_Y = remake_test_set(raw_test_X, raw_test_Y)
...
if RNNfunc == 'train':
    model.save(RNNcellname+'_'+TFILE[:-1]+'_'+af+'_'+str(predict_d)
               +'_' + str(Tstep)+'_'+str(Tx)+'_'+str(n_a)+'_'+str(RNNdropout)+'_'+str(RNNlayer)
               +'_'+TFILE2[:-1]+'_'+str(XXX)+'_'+str(YYY)+'_'+str(XXXYYY2)+'.h5'   )
    exit(0)
...
Out = model.predict(train_X, verbose=0)
...
def print_to_file(filename, RealY, PredY, RPredY) :
    ...
    print('Training with the following parameters: ', file=sample)
    print('  RNNcellname =', RNNcellname, file=sample)
    print('  RNN_Cell1 = ', RNN_cell1, file=sample)
    print('  RNN_Cell2 = ', RNN_cell2, file=sample)
    print('  Activation func = ', af, file=sample)
    print('  Tx = ', Tx, file=sample)
    print('  n_a = ', n_a, file=sample)
    print('  predict_d = ', predict_d, file=sample)
    print('  Tstep = ', Tstep, file=sample)
    print('  Tcutoff = ', Tcutoff, file=sample)
    print('  Sstep = ', Sstep, file=sample)
    print('  batch_size = ', batch_size, file=sample)
    print('  epoches = ', epoches, file=sample)
    print('  TRAIN directory =', TRAIN + TFILE, file=sample)
    print('  percentage XXX =', XXX, file=sample)
    print('  percentage YYY =', YYY, file=sample)

    if TFILE2 =='':
        print('  No separate testing directory.', file=sample)
    else : 
        print('  Testing directory = ', TRAIN+TFILE2, file=sample)
        if XXXYYY2 != 0 :
            print('  Testing directory uses percentage.', file=sample)
        else :
            print('  Testing directory use all teseting.', file=sample)

    print('  RNNdropout = ', RNNdropout, file=sample)
    print('  RNNlayer = ', RNNlayer, file=sample)
    print('  RNNfunc = ', RNNfunc, file=sample)
    ...
    for i in range(len(fog)-1) :
        if pre[i+1] != 10000 and pre[i+1] > fog[i+1][0] - fog[i][1] :
            pre[i+1] = fog[i+1][0] - fog[i][1]
    ...
    print('total duration: ', RealY.shape[0], ', total alarm duration: ', aa, file=sample) 
    print('total false alarm duration: ', totalfal, ', ratio: ', totalfal/aa, file=sample) 

    print('A total of ', fa, ' false alarms, duration : ', totalfal, file=sample)
...
print_to_file(TFILE[:-1]+'_'+str(predict_d)+'_'+str(Tstep)+'_'+RNNcellname+'_'+str(XXXYYY2)+'_'+'output.txt', NYYY, NOut, NOut1)
