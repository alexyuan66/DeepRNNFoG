...
if TFILE2 != '' and TFILE2 != TFILE :
    ...
    if RNNfunc == 'transfer1' or RNNfunc == 'transfer2' :
        train_X = train_X1
        train_Y = train_Y1
...
if RNNfunc != 'transfer1' and RNNfunc != 'transfer2' and RNNfunc != 'load' :
...
if RNNfunc == 'load' or RNNfunc == 'transfer1' :  
    ...
    if RNNfunc == 'transfer1' : 
        ...
        model.summary()
elif RNNfunc == 'transfer2' : # this only does the two layer case in the paper
    model1 = Sequential()
...
def print_to_file(filename, RealY, PredY) :
    sample = open(filename, 'w')
    for i in range (RealY.shape[0]):
        print(i, ': (', RealY[i][0], ', ', RealY[i][1], 
              '), (', PredY[i][0], ', ', PredY[i][1], ')', file=sample)
    sample.close()
  
print_to_file('output.txt', NYYY, NOut)
