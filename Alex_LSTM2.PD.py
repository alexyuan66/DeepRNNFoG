...
# The code is modified from Alex_RNN_PD_v1. Created on May 15 2020
#
...
Tcutoff = 6000 # Examine at most Tcutoff*0.015 seconds before FoG
Sstep = 6 # sample step is Sstep
...
model.fit([XXX, a0, c0], list(YYY.transpose((1, 0, 2))), batch_size=128,
...
Nc1=0
Nc2=0
for i in range(NOut1[0].shape[0]):
    if NYYY[i][Tx-1][0] == 0:
        Nc0 += 1
    if NOut1[Tx-1][i][0] != NYYY[i][Tx-1][0]:
        if NOut1[Tx-1][i][0] == 0:
            Nc1 += 1
        else : 
            Nc2 += 1
