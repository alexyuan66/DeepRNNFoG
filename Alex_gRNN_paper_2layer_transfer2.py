...
# To run the program:
#
# <draco> python3 Alex_gRNN_paper_2layer_transfer.py [SimpleRNN|LSTM|GRU] [acti] [n_a] [Tx] [predict_d] [Tstep] [Tcutoff] [Sstep]
#
# This implements the general RNN program in using Keras in the paper.
# The program is modified from Alex_gRNN_Paper_2layer.py
#
#
...
TRAIN = '../../dataset_fog_release/dataset/AllbutS01/'
TARGET = '../../dataset_fog_release/dataset/S01/'
...
def load_data(file_paths, shuffle = 1, XXX=2, YYY=3):
    global e_count
    train = []
    test = []
    train_X = []
...
#model1.add(RNN_cell2)
model1.add(densor)              
model1.load_weights('GRU_50_128_AllbutS01.h5')


#model = load_model('GRU_50_128_AllbutS01.h5')
...
Out = model.predict(test_X, verbose=0)
